...
export const fetchCartData = createAsyncThunk( //API Fetch
    ...
    async (_, { rejectWithValue }) => {
        try {
            ...
            // console.log(token);              //  getting token now
            
            if (!token) throw new Error('No auth token found in cookies');

            const response = await axios.post(
                'http://localhost:8000/web/cart/view-cart',
                {},
                {   
                    headers: {
            ...
            return response.data.userCartData ?? [];         // viewCart wala controller response
        } catch (error) {
...
export let cartSlice = createSlice(
    {
        ...
        initialState: {
            cart: [],
            loading: true,                   //  loading pahle se true rakhna hai
            error: null,
        },
        reducers: {
